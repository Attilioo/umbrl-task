const reactComponent = () => {
  return (
    <footer className="w-full bg-[#161C2D] flex flex-col items-center justify-center">
      <div className="w-4/5  flex mx-auto justify-between flex-col lg:items-center lg:flex-row">
        <div className="py-14">
          <p className="text-sm text-[#ffffff9a] mb-8">Company</p>
          <ul>
            <li className="text-base text-white my-4">About us</li>
            <li className="text-base text-white my-4">Contact us</li>
            <li className="text-base text-white my-4">Careers</li>
            <li className="text-base text-white my-4">Press</li>
          </ul>
        </div>
        <div className=" py-14">
          <p className="text-sm text-[#ffffff9a] mb-8">Product</p>
          <ul>
            <li className="text-base text-white my-4">Features</li>
            <li className="text-base text-white my-4">Pricing</li>
            <li className="text-base text-white my-4">News</li>
            <li className="text-base text-white my-4">Help Desk</li>
            <li className="text-base text-white my-4">Support</li>
          </ul>
        </div>
        <div className="py-14">
          <p className="text-sm text-[#ffffff9a] mb-8">Services</p>
          <ul>
            <li className="text-base text-white my-4">Digital Marketing</li>
            <li className="text-base text-white my-4">Content Writing</li>
            <li className="text-base text-white my-4">SEO for Business</li>
            <li className="text-base text-white my-4">UI Design</li>
          </ul>
        </div>
        <div className="py-14">
          <p className="text-sm text-[#ffffff9a] mb-8">Legal</p>
          <ul>
            <li className="text-base text-white my-4">Privacy Policy</li>
            <li className="text-base text-white my-4">Terms & Conditions</li>
            <li className="text-base text-white my-4">Return Policy</li>
          </ul>
        </div>
        <div className="py-14">
          <p className="text-sm text-[#ffffff9a] mb-8">Contact us</p>
          <ul>
            <li className="text-base text-white my-4">support@brainwave.io</li>
            <li className="text-base text-white my-4">+133-394-3439-1435</li>
          </ul>
        </div>
      </div>
      <div className="w-4/5 py-5 border-t-2 border-white border-opacity-10 flex justify-between">
        <p className="text-[#ffffff7e]">
          © 2024 Copyright, All Right Reserved, Made by Seju_ui_ux with 💙
        </p>

        <div className="flex gap-10">
          <a href="">
            <svg
              xmlns="http://www.w3.org/2000/svg"
              width="24"
              height="24"
              fill="white"
              viewBox="0 0 24 24"
            >
              <path d="M24 4.557c-.883.392-1.832.656-2.828.775 1.017-.609 1.798-1.574 2.165-2.724-.951.564-2.005.974-3.127 1.195-.897-.957-2.178-1.555-3.594-1.555-3.179 0-5.515 2.966-4.797 6.045-4.091-.205-7.719-2.165-10.148-5.144-1.29 2.213-.669 5.108 1.523 6.574-.806-.026-1.566-.247-2.229-.616-.054 2.281 1.581 4.415 3.949 4.89-.693.188-1.452.232-2.224.084.626 1.956 2.444 3.379 4.6 3.419-2.07 1.623-4.678 2.348-7.29 2.04 2.179 1.397 4.768 2.212 7.548 2.212 9.142 0 14.307-7.721 13.995-14.646.962-.695 1.797-1.562 2.457-2.549z" />
            </svg>
          </a>
          <a href="">
            <svg
              width="19"
              height="19"
              viewBox="0 0 19 19"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                d="M19 9.5573C18.9992 4.4726 15.0424 0.278485 9.99592 0.0132214C4.94945 -0.252042 0.579855 3.50441 0.0525261 8.56134C-0.474803 13.6183 3.02473 18.2055 8.01601 19V12.3203H5.60428V9.5573H8.01601V7.45142C8.01601 5.0558 9.43443 3.73253 11.6039 3.73253C12.3164 3.74247 13.0272 3.80477 13.7307 3.91892V6.27153H12.5325C12.1189 6.21626 11.7028 6.35325 11.4017 6.64381C11.1006 6.93437 10.9471 7.34697 10.9847 7.76506V9.5573H13.6196L13.1981 12.3203H10.9847V19C15.6013 18.2651 19.0007 14.2603 19 9.5573Z"
                fill="white"
              />
            </svg>
          </a>
          <a href="">
            <svg
              width="18"
              height="18"
              viewBox="0 0 18 18"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M5.28937 0.054C6.24937 0.0105 6.55575 0 9 0C11.4443 0 11.7506 0.0105 12.7106 0.054C14.1739 0.12075 15.4583 0.47925 16.4895 1.5105C17.5211 2.54212 17.8792 3.82687 17.946 5.28937C17.9895 6.24937 18 6.55575 18 9C18 11.4443 17.9895 11.7506 17.946 12.7106C17.8792 14.1739 17.5208 15.4583 16.4895 16.4895C15.4579 17.5211 14.1727 17.8792 12.7106 17.946C11.7506 17.9895 11.4443 18 9 18C6.55575 18 6.24937 17.9895 5.28937 17.946C3.82612 17.8792 2.54175 17.5208 1.5105 16.4895C0.478875 15.4579 0.12075 14.1731 0.054 12.7106C0.0105 11.7506 0 11.4443 0 9C0 6.55575 0.0105 6.24937 0.054 5.28937C0.12075 3.82612 0.47925 2.54175 1.5105 1.5105C2.54212 0.478875 3.82687 0.12075 5.28937 0.054ZM12.6368 1.674C11.6876 1.6305 11.403 1.6215 9 1.6215C6.597 1.6215 6.31238 1.63087 5.36325 1.674C4.33913 1.72088 3.3885 1.926 2.65725 2.65725C1.926 3.3885 1.72088 4.33913 1.674 5.36325C1.6305 6.31238 1.6215 6.597 1.6215 9C1.6215 11.403 1.63087 11.6876 1.674 12.6368C1.72088 13.6609 1.926 14.6115 2.65725 15.3427C3.3885 16.074 4.33913 16.2791 5.36325 16.326C6.312 16.3695 6.59663 16.3785 9 16.3785C11.4034 16.3785 11.688 16.3691 12.6368 16.326C13.6609 16.2791 14.6115 16.074 15.3427 15.3427C16.074 14.6115 16.2791 13.6609 16.326 12.6368C16.3695 11.6876 16.3785 11.403 16.3785 9C16.3785 6.597 16.3691 6.31238 16.326 5.36325C16.2791 4.33913 16.074 3.3885 15.3427 2.65725C14.6115 1.926 13.6609 1.72088 12.6368 1.674Z"
                fill="white"
              />
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M4.90918 9.40909C4.90918 6.92394 6.92366 4.90909 9.40918 4.90909C11.8947 4.90909 13.9092 6.92358 13.9092 9.40909C13.9092 11.8946 11.8947 13.9091 9.40918 13.9091C6.92366 13.9091 4.90918 11.8942 4.90918 9.40909ZM6.48805 9.40909C6.48805 11.0223 7.79599 12.3302 9.40918 12.3302C11.0224 12.3302 12.3303 11.0223 12.3303 9.40909C12.3303 7.7959 11.0224 6.48796 9.40918 6.48796C7.79599 6.48796 6.48805 7.7959 6.48805 9.40909Z"
                fill="white"
              />
              <circle cx="13.5002" cy="4.5" r="1.22727" fill="white" />
            </svg>
          </a>
          <a href="">
            <svg
              width="18"
              height="18"
              viewBox="0 0 18 18"
              fill="none"
              xmlns="http://www.w3.org/2000/svg"
            >
              <path
                fill-rule="evenodd"
                clip-rule="evenodd"
                d="M0.75 0H17.25C17.6625 0 18 0.3375 18 0.75V17.25C18 17.6625 17.6625 18 17.25 18H0.75C0.3375 18 0 17.6625 0 17.25V0.75C0 0.3375 0.3375 0 0.75 0ZM2.6625 15.3375H5.325V6.75H2.6625V15.3375ZM4.0125 5.5875C3.15 5.5875 2.475 4.9125 2.475 4.05C2.475 3.1875 3.15 2.5125 4.0125 2.5125C4.875 2.5125 5.55 3.1875 5.55 4.05C5.55 4.875 4.875 5.5875 4.0125 5.5875ZM12.675 15.3375H15.3V10.6125C15.3 8.2875 14.8125 6.525 12.1125 6.525C10.8375 6.525 9.9375 7.2375 9.6 7.9125H9.5625V6.75H7.0125V15.3375H9.675V11.1C9.675 9.975 9.9 8.8875 11.2875 8.8875C12.675 8.8875 12.675 10.1625 12.675 11.175V15.3375Z"
                fill="white"
              />
            </svg>
          </a>
        </div>
      </div>
    </footer>
  )
}

export default reactComponent
